...
namespace Common
{
    ...
    public class ObjectPool<T> where T : Component
    {
        ...
        private readonly Dictionary<T, Pool> pools = new();
        //словарь пуллов по инстанс id, здесь храним только пуллы включенных инстансов, чтобы корректно вернуть в нужный
        private readonly Dictionary<int, Pool> poolsByInstances = new();
        //хэшсет всех инстансов пула, чтобы при повторном возврате в пул понимать, что это наш инстанс и ничего делать не нужно
        private readonly HashSet<int> instancesIds = new();
        ...
        public T InstantiateFromPool(T prefab)
        {
            if (pools.TryGetValue(prefab, out var pool))
            {
                ...
                for (int i = 0; i < poolItems.Count; ++i)
                {
                    ...
                    if (poolsByInstances.ContainsKey(id) != true)
                    {
                        poolsByInstances[id] = pool;
                        return instance;
                    }
                }
        ...
        public void Release(T instance)
        {
            if (instance != null)
            {
                ...
                else if (!instancesIds.Contains(id))
                {
